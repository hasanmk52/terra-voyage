...
>(({ className, ...props }, ref) => (
  <SwitchPrimitive.Root
    className={cn(
      ...
      // Track colors: light gray when off, light blue when on
      "data-[state=unchecked]:bg-gray-200 dark:data-[state=unchecked]:bg-gray-700",
      "data-[state=checked]:bg-blue-400 dark:data-[state=checked]:bg-blue-500",
      // Subtle border for visibility in cards
      "border-gray-300 data-[state=checked]:border-blue-400 dark:border-gray-600 dark:data-[state=checked]:border-blue-500",
      className
    )}
    {...props}
    ref={ref}
  >
    <SwitchPrimitive.Thumb
      className={cn(
        // High-contrast thumb
        "pointer-events-none block h-5 w-5 rounded-full bg-white dark:bg-gray-900 shadow-md ring-0 transition-transform data-[state=checked]:translate-x-5 data-[state=unchecked]:translate-x-0"
      )}
    />
  </SwitchPrimitive.Root>
));
