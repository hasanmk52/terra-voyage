...
export function StatusManager({
  ...
  const handleStatusChange = async (newStatus: TripStatus, skipConfirmation = false) => {
    if (!skipConfirmation) {
      ...
      setConfirmation({
        ...
        description: `Are you sure you want to change the trip status to ${transition.status.toLowerCase()}? ${transition.description}`,
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to update trip status. Please try again.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  if (transitions.length === 0) {
    return (
      <div className="flex items-center gap-2">
        <StatusBadge status={currentStatus} showTooltip />
        {showLabel && (
  ...
  return (
    <>
      <div className={`flex items-center gap-2 ${className}`}>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              ...
              aria-label={`Change trip status from ${currentStatus.toLowerCase()}. Currently ${transitions.length} option${transitions.length !== 1 ? 's' : ''} available`}
              aria-haspopup="menu"
              aria-expanded={false}
            >
              {isLoading && <Loader2 className="w-3 h-3 mr-1.5 animate-spin" aria-hidden="true" />}
              Change Status
          ...
          <DropdownMenuContent align="end" className="min-w-[200px]" role="menu">
            ...
            {transitions.map((transition) => (
              <DropdownMenuItem
                ...
                role="menuitem"
                aria-label={`Change status to ${transition.status.toLowerCase()}: ${transition.description}`}
              >
                <StatusBadge 
        ...
        {showLabel && (
          <span className="text-sm text-gray-500">
            {transitions.length} option{transitions.length !== 1 ? 's' : ''} available
          </span>
        )}
      </div>
      ...
      <AlertDialog 
        ...
        onOpenChange={(open) => setConfirmation(prev => ({ ...prev, show: open }))}
      >
        <AlertDialogContent className="bg-white border border-gray-200 shadow-xl max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-gray-600 mt-2">
              {confirmation.description || 'Are you sure you want to change the trip status?'}
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          <AlertDialogFooter className="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 gap-2">
            ...
            <AlertDialogAction
              onClick={() => confirmation.newStatus && handleStatusChange(confirmation.newStatus, true)}
              disabled={isLoading}
              className={`focus:ring-2 focus:ring-offset-2 text-white ${confirmation.variant === 'destructive' ? 'bg-red-600 hover:bg-red-700 focus:ring-red-500' : 'bg-blue-600 hover:bg-blue-700 focus:ring-blue-500'}`}
