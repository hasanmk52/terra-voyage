...
export async function POST(request: NextRequest) {
  try {
    ...
    if (useMocks) {
      await simulateDelay('calendar')
      
      const body = await request.json()
      const { tripId, format, activityId, options } = calendarExportSchema.parse(body)
      
      // Generate mock calendar content
      const mockCalendar = `BEGIN:VCALENDAR
VERSION:2.0
PRODID:-//Terra Voyage//Trip Itinerary//EN
BEGIN:VEVENT
UID:mock-event-${tripId}@terravoyage.com
...
END:VCALENDAR`
      ...
      const filename = `trip-${tripId}-calendar.ics`
      
      return new NextResponse(mockCalendar, {
        status: 200,
        headers: {
